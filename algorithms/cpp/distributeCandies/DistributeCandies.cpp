...
// 300 ms, faster than 74.11% of C++, 51.1 MB, less than 49.10% of C++
class Solution {
public:
    int distributeCandies(vector<int>& candies) {
        int maxcan = candies.size()/2;
        unordered_map<int,bool> hashmap;
        for(auto i:candies)hashmap[i] = true;
        int maxtype = hashmap.size();
        return min(maxcan,maxtype);
    }
};