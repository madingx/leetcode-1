...
* any square (including the target) at the same time as a ghost, it doesn't count as an escape.
...
* Output: true
* Explanation: 
* You can directly reach the destination (0, 1) at time 1, while the ghosts located at
*  (1, 0) or (0, 3) have no way to catch up with you.
...
// 8 ms, faster than 60.65% of C++, 9.4 MB, less than 100.00% of C++
class Solution {
public:
    bool escapeGhosts(vector<vector<int>>& ghosts, vector<int>& target) {
        
        // calculate manhattan distance between every ghost and the target, if that is less than or equal to player's distance from target, 
        // it is not possible
        // think like this, the ghost will reach target before us and will catch us there
        
        for(auto &g: ghosts) {
            if(abs(g[0] - target[0]) + abs(g[1] - target[1]) <= abs(target[0]) + abs(target[1])) return false;
        }
        return true;
    }
};