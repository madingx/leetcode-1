...
// 24 ms, faster than 83.62% of C++， 10.4 MB, less than 45.24% of C++
class Solution {
public:
    void floodFillResc(vector<vector<int>>& image, int sr, int sc, int &newColor,int &origin) {
        if( sr<0 || sr>= image.size() )return;   
        if( sc<0 || sc>= image[sr].size() )return;
        if( image[sr][sc] != origin )return;
        if( image[sr][sc] == newColor )return;
        image[sr][sc] = newColor;
        floodFillResc(image, sr+1, sc, newColor,origin);
        floodFillResc(image, sr-1, sc, newColor,origin);  
        floodFillResc(image, sr, sc+1, newColor,origin);  
        floodFillResc(image, sr, sc-1, newColor,origin);  
        return;
    ...
    vector<vector<int>> floodFill(vector<vector<int>>& image, int sr, int sc, int newColor) {
        ...
        floodFillResc(image, sr, sc, newColor,origin); 
        return image;
    }
};