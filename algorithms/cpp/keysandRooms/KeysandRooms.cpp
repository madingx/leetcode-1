...
// 16 ms, faster than 35.97% of C++, 11.5 MB, less than 18.84% of C++
class Solution {
    set<int> keys;
public:
    void visitResc(vector<vector<int>>& rooms,int roomnum){
        for(auto k : rooms[roomnum]){
            auto flag = keys.insert(k);
            if(flag.second)
                visitResc(rooms,k);
        }
        return;
...
// BFS
// 12 ms, faster than 89.78% of C++, 11.2 MB, less than 36.82% of C++
class Solution {
public:
    bool canVisitAllRooms(vector<vector<int>>& rooms) {
