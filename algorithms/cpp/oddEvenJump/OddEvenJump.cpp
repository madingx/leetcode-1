 ...
 * A[i] >= A[j] and A[j] is the largest possible value.  If there are multiple such 
 * indexes j, you can only jump to the smallest such index j.
 ...
 * During our 1st jump (odd numbered), we first jump to i = 1 because A[1] is the smallest 
 * value in (A[1], A[2], A[3], A[4]) that is greater than or equal to A[0].

 * During our 2nd jump (even numbered), we jump from i = 1 to i = 2 because A[2] is the largest 
 * value in (A[2], A[3], A[4]) that is less than or equal to A[1].  A[3] is also the largest 
 * value, but 2 is a smaller index, so we can only jump to i = 2 and not i = 3.

 * During our 3rd jump (odd numbered), we jump from i = 2 to i = 3 because A[3] is the smallest 
 ...
 * In total, there are 3 different starting indexes (i = 1, i = 3, i = 4) where we can reach 
 * the end with some number of jumps.
...
// 96 ms, faster than 87.75% of C++, 19.2 MB, less than 48.48% of C++
class Solution {
public:
...
public:
    int oddEvenJumps(vector<int>& A) {
        ...
        int cnt = 1;
        vector<bool> odds(A.size(),false),evens(A.size(),false);
        odds[A.size() - 1] = true;
        evens[A.size() - 1] = true;
        int lastodd = A.size()-1,lasteven = A.size()-1;
        for(int i=A.size()-1; i>=0; i--){
            int smallodd = i;            
            for(int j = i+1;j<A.size();j++){
                if( A[i] <= A[j]){
                    if(smallodd == i || A[j] < A[smallodd])smallodd = j;
                    if(A[i] == A[j])break;
                }
            }
            if(smallodd != i && evens[smallodd] ){odds[i] = true;cnt++;}
            
            int largeeven = i;
            for(int j = i+1;j<A.size();j++){
                if( A[i] >= A[j]){
                    if(largeeven == i || A[j] > A[largeeven])largeeven = j;
                    if(A[i] == A[j])break;
                }
            }
            if(largeeven != i && odds[largeeven] )evens[i] = true;
